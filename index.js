...
app.post('/loggingin', async (req,res) => {
	...
	const result = await userCollection.find({username: username}).project({username: 1, password: 1}).toArray();
...
app.post('/promote', async (req, res) => {
	var username = req.body.username;
	//const db = database.db('aju');
	await db.collection("users").updateOne({username: username}, {$set: {user_type: 'admin'}});
...
app.post('/demote', async (req, res) => {
	var username = req.body.username;
	//const db = database.db('aju');
	await db.collection("users").updateOne({username: username}, {$set: {user_type: 'user'}});
	res.send("User has been demoted to user.");
});
